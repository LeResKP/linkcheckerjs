    ...
    if (response.stage === 'end') {
        if (response.url.substr(0, 5) !== 'data:' && url.substr(0, 8) === 'https://' && response.url.substr(0, 8) !== 'https://') {
            if (300 <= response.status && response.status < 400) {
                // Just a warning it redirects to a unsecure resource
                logger.warning(url, response.url, response.status, 'Unsecure resource');
            }
            else {
        ...
        if (response.status === null && response.url.substr(0, 5) === 'data:') {
            ...
            logger.warning(response.url, response.redirectURL, response.status, response.statusText);
        }
        else {
...
page.open(url, function(status) {
    if (status === "success") {
        ...
        console.log(JSON.stringify({'res': logger.messages, 'links': links}, undefined, 4));
        phantom.exit(0);
    }
    else {
